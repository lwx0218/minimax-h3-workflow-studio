...
<head>
  ...
  <title>MiniMax H3 Studio — R3</title>
  <style>
...
<body>
<main>
  <h1>MiniMax H3 Studio — Single Worker</h1>
  <p class=\"muted\">Guided Mode submits native ComfyUI H3 API workflows. Advanced Canvas opens the pinned ComfyUI frontend.</p>
  <div class=\"grid\">
    <section>
      <h2>Guided Mode</h2>
      <form id=\"runForm\">
        ...
        <label>Generation Profile</label><select name=\"profile\"><option value=\"balanced-r2-a5000\">Balanced — R2 A5000 proven route</option></select>
      ...
      <p>Advanced Canvas: <a id=\"canvasLink\" target=\"_blank\">open full ComfyUI frontend</a></p>
      <p class=\"muted\">Load UI workflows from <code>workflows/comfy-ui/</code> inside the Advanced Canvas.</p>
    </section>
    <section>
...
async function init(){ const s=await api('/api/config'); document.getElementById('canvasLink').href=s.advanced_canvas_url; drawRecent(); }
...
class StudioService:
    def __init__(self, config: StudioConfig):
        ...
        self.comfy = ComfyClient(config.worker_url)

    def health(self) -> dict[str, Any]:
        worker = None
        try:
            worker = self.comfy.system_stats()
        except Exception as exc:  # noqa: BLE001
            worker = {"error": repr(exc)}
        return {"ok": "error" not in worker, "worker_url": self.config.worker_url, "worker": worker}
    ...
    def submit(self, fields: dict[str, str], files: dict[str, Path]) -> dict[str, Any]:
        ...
        if kind == "fl2va_first_frame":
            ...
            upload = self.comfy.upload_image(local, upload_name=upload_name)
        ...
        submit = self.comfy.submit_prompt(api_prompt, client_id=client_id)
    ...
    def refresh(self, run_id: str) -> dict[str, Any]:
        ...
        if not prompt_id:
            return self.decorate(record)
        try:
            hist = self.comfy.history(str(prompt_id))
        except Exception as exc:  # noqa: BLE001
            ...
            if status.get("completed") is True or status.get("status_str") == "success":
                ...
                for idx, item in enumerate(flatten_history_outputs(entry)):
                    if str(item.get("filename", "")).lower().endswith((".mp4", ".webm", ".mkv", ".mov")):
                        artifacts.append({"artifact_id": f"a{idx}", **item, "view_url": self.comfy.view_url(item)})
    ...
    def cancel(self, run_id: str) -> dict[str, Any]:
        record = self.store.read(run_id)
        res = self.comfy.interrupt()
        record["status"] = "cancelled"
        record["cancel_response"] = res
        record["updated_at"] = utc_now()
        self.store.write(record)
        return self.decorate(record)
    ...
    def decorate(self, record: dict[str, Any]) -> dict[str, Any]:
        ...
        copy["worker_ws_url"] = self.comfy.websocket_url
        copy["advanced_canvas_url"] = self.config.worker_url + "/"
        return copy
    ...
    def proxy_artifact(self, run_id: str, artifact_id: str) -> tuple[str, bytes]:
        ...
        for item in record.get("artifacts", []):
            if item.get("artifact_id") == artifact_id:
                with urllib.request.urlopen(self.comfy.view_url(item), timeout=120) as resp:
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        try:
            ...
            if parsed.path == "/api/config":
                return self.send_json({"advanced_canvas_url": self.service.config.worker_url + "/", "worker_url": self.service.config.worker_url, "profiles": self.service.config.profiles})
...
def run_server(config: StudioConfig | None = None) -> None:
    ...
    print(f"Advanced Canvas worker: {cfg.worker_url}")
    httpd.serve_forever()
