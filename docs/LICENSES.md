...
Status: inventory for the local single-user MVP. Engineering evidence, not legal advice.

| Component | Role | Identity | License / posture |
|---|---|---|---|
| Project-owned `h3_studio/`, scripts, docs, configs | Control plane, Guided Mode, scripts | repository candidate | Project-owned unless otherwise declared by Owner |
...
| PyTorch/CUDA wheel stack | Runtime compute | `torch 2.14.0+cu130`, `torchvision 0.29.0+cu130`, `torchaudio 2.11.0+cu130`, CUDA wheel packages | Third-party binary/runtime licenses; locked in `config/requirements-lock.txt` |
| H3 runtime Python packages | H3 execution dependencies | `comfy-kitchen 0.2.31`, `comfy-aimdo 0.4.15`, `transformers 5.8.1`, `accelerate 1.14.0`, `av 18.1.0` | Third-party package licenses; locked in `config/requirements-lock.txt` |
